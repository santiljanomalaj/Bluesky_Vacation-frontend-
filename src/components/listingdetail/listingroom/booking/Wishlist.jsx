...
class Wishilist extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      wishlist_items: [],
  ...
  componentDidMount() {
    listingDetailService.getWishlistData(this.props.roomId).then(res => {
      if (res) {
        this.setState({
          wishlist_items: res
        });
  ...
  handleCreateWishlist() {
    listingDetailService.getWishlistCreate(this.state.wish_list_text, this.props.roomId)
      .then(res => {
        if (res) {
          this.setState({
            wishlist_items: res,
  ...
  handeSaveWishlist(index, wishlist_id, saved_id) {
    listingDetailService.getWishlistSave(this.props.roomId, saved_id, wishlist_id)
      .then(res => {
        if (res) {
          let list_items = this.state.wishlist_items;
          list_items[index].saved_id = res;
          this.setState({ wishlist_items: list_items });
        }
      });
  }
  render() {
    return (
      <div className="wishlist-wrapper">
        ...
        >
          <div className="row d-lg-flex flex-md-column flex-lg-row" id="wishlist-dialog">
            <div className="col-lg-5 d-flex flex-basis-40 p-0">
              <div id="review_form_wizard_left_form" className="left_form">
                <figure>
                  <img
                    src="https://cdn2.iconfinder.com/data/icons/christmas-1-14/96/Christmas-solid_Wishlist_-512.png"
                    width={100}
                    alt="wishlist"
                  />
                </figure>
                <h2>Wishlist</h2>
                <p>
                  Do you like this listing? You can add this listing to your
                  wishlist. So you can visit this page directly in your
                  Wishlist.
                </p>
                <a
                  href="https://www.vacation.rentals/united-states"
                  id="more_info"
                >
                  <i className="icon icon3-house" />
                </a>
              </div>
            </div>
            <div className="col-lg-7 d-flex justify-content-center p-4">
              <div className="wl-modal-wishlists w-100">
                ...
                <div className="panel-body panel-body-scroll wl-modal-wishlists__body wl-modal-wishlists__body--scroll">
                  <div className="text-lead text-gray space-4">
                    Save your favorite listings, compare notes, and start
                    planning your next adventure.
                  </div>
                  <div
                    className="wl-modal-wishlist-row clickable ng-scope text-dark-gray"
                    id="wishlist_row_0"
                  >
                    {this.state.wishlist_items &&
                    this.state.wishlist_items.length
                      ? this.state.wishlist_items.map((item, index) => {
                          return (
                            <div
                            ...
                            >
                              ...
                              <div className="va-middle text-right">
                                <div className="h3 wl-modal-wishlist-row__icons">
                                  {item.saved_id ? (
                                    <i
                                      className="icon icon-heart-alt icon-light-gray wl-modal-wishlist-row__icon-heart-alt"
                                      onClick={() =>
                                        this.handeSaveWishlist(
                                          index,
                                          item.id,
                                          item.saved_id
                                        )
                                    ...
                                    <i
                                      className="icon icon-heart icon-rausch wl-modal-wishlist-row__icon-heart"
                                      onClick={() =>
                                        this.handeSaveWishlist(
                                          index,
                                          item.id,
                    ...
                    <strong>
                      <div className="pull-left text-lead va-container va-container-v">
                        <input
                          ...
                          onChange={this.handleChangeWishListText}
                          autoComplete="off"
                          id="wish_list_text"
                          value={this.state.wish_list_text}
                      ...
                      <div className="pull-right">
                        <button
                          id="wish_list_btn"
                          className="btn btn-flat wl-modal-wishlists__footer__save-button btn-contrast"
        ...
        <div
          className="rich-toggle wish_list_button not_saved"
          data-hosting_id={11449}
        >
          <button
            type="button"
          ...
          >
            {this.state.wishlist_items.length ? (
              ...
              <span className="rich-toggle-unchecked">
                <i className="icon icon-heart-alt icon-light-gray" />
