...
class Amenities extends React.PureComponent {
  ...
  // }
  render() {
    let amenities_list = this.props.amenities_type
      ? this.props.amenities_type
      : [];
    amenities_list = amenities_list.map(amenity_type => {
      let amenties_items = this.props.amenities.map((amenity, index) => {
        if (amenity && amenity.type_id && amenity.type_id === amenity_type.id) {
          return (
            <div
              className="row-space-2 col-sm-12  col-md-6 space-sm-0"
              key={index}
            >
      ...
      return (
        <div className="lang-chang-label clrleft" key={amenity_type.id}>
    ...
    return (
      <div className="expandable-content-summary">
