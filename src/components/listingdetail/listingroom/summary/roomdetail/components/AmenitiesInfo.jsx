    ...
    bathroom_list = bathroom_list.map(bathroom => {
      ...
      return (
        <li key={bathroom.id}>
          ...
          <label>
            type {bathroom.bathroom_type}{" "}
            {bathroom.bathfeature ? bathroom.bathfeature : ""}
          </label>
        </li>
      );
    });
      ...
      <div className="row">
        <div className="col-md-12 space-top-sm-2 lang-chang-label col-sm-12">
          <div className="info_title">
            Property Id:{" "}
            <strong>
              {this.props.room_detail ? this.props.room_detail.id : "Nan"}
            </strong>
          </div>
          <ul className="right-space rj_the_space">
            <li>
              <div className="rj_list_head">
                <span>Bedrooms: </span>
              </div>
              <ul className="rj_list_dist">{bedroom_list}</ul>
            </li>
            <li>
              <div className="rj_list_head">
                <span>Bathrooms: </span>
              </div>
              <ul className="rj_list_dist">{bathroom_list}</ul>
            </li>
          </ul>
          {this.props.room_detail && this.props.room_detail.bed_type_name ? (
            <div className="info_title">
              Property type:{" "}
              <strong>{this.props.room_detail.bed_type_name}</strong>
            </div>
          ) : (
            ""
          )}
          <div className="info_title">
            ...
            <strong>
              {" "}
              {this.props.room_detail ? this.props.room_detail.bedrooms : ""}
            </strong>
          </div>
          <div className="info_title">
            property_type:{" "}
            <strong>
              {this.props.room_detail
                ? this.props.room_detail.property_type_name
                : ""}{" "}
            </strong>
          </div>
          <div className="info_title">
