...
class RoomFeedback extends React.PureComponent {
  ...
  componentDidMount() {
    listingDetailService.getHomeReview(this.props.room_id).then(res => {
      if (res) {
        this.setState({
          reviews: res.reviews
        });
  ...
  render() {
    return (
      <div className="row">
        <span style={anchor_style} id="reviews">
          {" "}
        </span>
            ...
            <div className="room_admin row">
              <div className="col-md-3  space-sm-4 text-center space-sm-2 lang-chang-label">
                <div className="media-photo-badge">
                  <img
                    alt="User Profile"
                    className="host-profile-image img-responsive rounded-circle"
                    src={
                      this.props.user_details.user_profile_pic
                        ? this.props.user_details.user_profile_pic
                        : null
                    }
                    title="Stefanie"
                  ...
                  <Link to="#" className="link-reset text-wrap">
                    {this.props.user_details.full_name
                      ? this.props.user_details.full_name
                      : ""}
                  </Link>
                </div>
              ...
              <div className="col-md-9 ">
                <h1
                  itemProp="name"
                  className="overflow h3 row-space-1 text-center-sm"
                  id="listing_name"
                >
                ...
                <div
                  id="display-address"
                  className="row-space-2 text-muted text-center-sm"
