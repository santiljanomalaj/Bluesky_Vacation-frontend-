...
class YourTrip extends React.PureComponent {
  constructor(props) {
    super(props)
    this.state = {
      page_data: {
  ...
  componentDidMount() {
    dashboardService.geCurrTripList().then(res => {
      if (res) {
        this.setState({
          page_data : res.page_data
        });
      } else {
        alertService.showError('Get current trip list');
      }
    })
  }
              ...
              <form acceptCharset="UTF-8" onSubmit={(event) => this.handleCancelSubmit(event, index, status)} id="cancel_reservation_form" method="post" name="cancel_reservation_form">
                <div className="panel-body p-0">
                  <div id="decline_reason_container">
                    <p> Help us improve your experience. What's the main reason for Cancelling this Reservation? </p>
                    <p> <strong> Your response is not shared with the host </strong> </p>
                    <div className="select">
                      <select id="cancel_reason" name="cancel_reason">
                        <option value>Why are you declining </option>
                        <option value="no_longer_need_accommodations">I no longer need accommodations </option>
                        <option value="travel_dates_changed">My travel have dates changed </option>
                        ...
                        <option value="my_host_needs_to_cancel">My host need to cancel </option>
                        <option value="uncomfortable_with_the_host">I'm uncomfortable with the host </option>
                        <option value="place_not_okay">The place is not what I expected </option>
  ...
  render() {
    ...
    page_content.push(
      <div className="panel row-space-4" key='current_trips'>
        <div className="panel-header"> Current Trips </div>
      ...
      return (
        <tr key={trip.id}>
          ...
          <td>
            <ul className="list-unstyled">
              <li className="row-space-1">
                <Link to="#" className="button-steel" title='Are you sure that you want to cancel the request? Any money transacted will be refunded.' onClick={(event) => this.handleCancelConfirmModalOpen(event, index, trip.status)} >Cancel Request
                </Link>
              </li>
            </ul>
          </td>
